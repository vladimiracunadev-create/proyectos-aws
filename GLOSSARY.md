...
- **STS (Security Token Service)**: Servicio web de AWS que permite solicitar credenciales temporales de privilegios limitados.
- **CloudFront Invalidation**: Proceso de limpiar la caché de los nodos de borde (edge locations) para servir contenido nuevo.
...
- **Inmutabilidad**: Concepto donde los despliegues no se modifican una vez realizados; se crea una nueva versión.
...
- **GitOps**: Metodología que utiliza Git como la "única fuente de verdad" para la infraestructura y las aplicaciones.
- **JWT (JSON Web Token)**: Estándar para transmitir información de forma segura entre partes como un objeto JSON (usado en OIDC).
- **CVE (Common Vulnerabilities and Exposures)**: Lista de fallos de seguridad informática de conocimiento público.
...
- **TTM (Time to Market)**: Tiempo que transcurre desde que se concibe un producto hasta que está disponible en el mercado.
- **RTO/RPO**: Objetivos de tiempo y punto de recuperación en estrategias de resiliencia ante desastres.
...
- **Glassmorphism**: Estilo de diseño basado en transparencias y desenfoques.
- **Lighthouse**: Herramienta de código abierto de Google para mejorar la calidad de las páginas web (Performance, SEO, PWA).
- **Hydration (Hidratación)**: Proceso en el que el JavaScript del lado del cliente añade interactividad al HTML renderizado.
