...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .learning-box strong { display: block; margin-bottom: 6px; color: #dff6ff; }
    .learning-box p { margin: 0; color: var(--muted); line-height: 1.5; }
...
<body>
  <main class="shell">
    <section class="hero">
      ...
      <p class="lead">
        Estas viendo la puerta segura de una app con usuarios. Aqui creas una cuenta,
        haces login, recibes un token y pruebas un endpoint privado. Eso resuelve el
        problema real de no dejar datos ni funciones internas abiertas a cualquiera.
        El enlace WAF agrega la segunda capa: frenar trafico sospechoso antes de la app.
      </p>
      <div class="hero-grid">
        <div class="mini-panel">
          <div class="stats">
            ...
            <div class="stat"><strong>Menos riesgo</strong><span>Sin crypto manual en Lambda</span></div>
          </div>
          <div class="chips">
        ...
        <div class="mini-panel">
          ...
          <div class="story">
            <article class="story-card">
              <strong>Que estas construyendo</strong>
              <p>La entrada segura de una aplicacion con usuarios: portal de clientes, panel interno o API privada.</p>
            </article>
            <article class="story-card">
              <strong>Que problema real resuelve</strong>
              <p>Evita que cualquiera consulte datos privados o ejecute funciones internas sin autenticarse primero.</p>
            </article>
            <article class="story-card">
              <strong>Que ganas con esta arquitectura</strong>
              <p>API Gateway valida el token antes de llegar a tu codigo. Menos riesgo y menos logica sensible en Lambda.</p>
            </article>
            <article class="story-card">
    ...
    <section class="panel">
      <h2>Prueba guiada: que hace este producto de verdad</h2>
      <p class="helper">Piensa en un portal de clientes o un panel interno. En estos tres pasos veras como AWS crea la identidad, emite la credencial y permite entrar solo al usuario valido.</p>
      <div class="learning-box">
        <strong id="learning-title">Que estas por demostrar</strong>
        <p id="learning-text">Una app con usuarios no deberia dejar entrar a cualquiera. Aqui probaras alta de usuario, login y acceso privado sin programar validacion criptografica manual.</p>
      </div>
    ...
    <section class="panel">
      ...
      <div class="story">
        <article class="story-card">
          <strong>Donde se usa en la vida real</strong>
          <p>Portales de clientes, dashboards internos, backoffice, apps moviles y APIs privadas donde no cualquiera deberia entrar.</p>
        </article>
        <article class="story-card">
          <strong>Lo que acabas de resolver</strong>
          <p>Registro, login y control de acceso con servicios administrados de AWS en vez de inventar autenticacion casera dentro de la Lambda.</p>
        </article>
        <article class="story-card">
          <strong>Por que existe una segunda URL</strong>
          <p>Porque AWS permite JWT Authorizer nativo en HTTP API, pero WAF se asocia a REST API. No es otra app: es la misma historia de seguridad, completada con perimetro.</p>
        </article>
      </div>
      <div class="waf-note">
  ...
  <script>
    ...
    function setLearning(title, text) {
      learningTitleEl.textContent = title;
      learningTextEl.textContent = text;
    }
    ...
    document.getElementById("btn-register").addEventListener("click", async () => {
      ...
      setLearning(
        ...
        "Estas creando una identidad real en Cognito. En una app productiva esto evita guardar usuarios o contrasenas de forma casera dentro de tu propio codigo."
      );
      ...
      if (status === 201) {
        ...
        setLearning(
          "Usuario creado en un servicio real",
          "Ya no dependes de usuarios inventados o hardcodeados. AWS Cognito se hace cargo del alta y la gestion basica de identidad."
        );
        ...
        setLearning(
          "El alta no paso",
          "El producto te esta mostrando un problema real de identidad. Sin un usuario valido, no puedes avanzar al acceso protegido."
        );
      }
    });
    ...
    document.getElementById("btn-login").addEventListener("click", async () => {
      ...
      setLearning(
        "Paso 2: emision de credencial",
        "Ahora AWS valida las credenciales y emite el token del usuario. Esa credencial digital es la que despues abre el endpoint privado."
      );
      ...
      if (status === 200) {
        ...
        setLearning(
          "Credencial emitida por AWS",
          "Acabas de obtener la prueba de identidad del usuario. En la vida real, este token sirve para abrir recursos privados sin que la app invente su propia autenticacion."
        );
        ...
        setLearning(
          "El login fue rechazado",
          "Esto tambien es parte del valor del producto: un usuario o contrasena incorrecta no deben abrir acceso a datos privados."
        );
      }
    });
    ...
    document.getElementById("btn-profile").addEventListener("click", async () => {
      ...
      setLearning(
        ...
        "Ahora veras si la API deja pasar al usuario valido. La idea es que la comprobacion del token ocurra antes de tu codigo de negocio."
      );
      ...
      if (status === 200) {
        ...
        setLearning(
          "Acceso privado resuelto",
          "Acabas de demostrar el problema central que este DEMO resuelve: solo el usuario autenticado entra a la ruta protegida, y API Gateway hizo la validacion antes de Lambda."
        );
      } else {
        ...
        setLearning(
          "Acceso denegado",
          "Si el token no es valido, la aplicacion debe cerrar el paso. Ese rechazo temprano es justamente parte de la seguridad que estas montando."
        );
      }
    });
    ...
    document.getElementById("btn-copy-token").addEventListener("click", async () => {
      ...
      try {
        ...
        setLearning(
          "Puente hacia la segunda capa",
          "El mismo usuario ya fue validado. Ahora usaras esa misma identidad en la URL WAF para entender la diferencia entre autenticacion y defensa perimetral."
        );
      } catch (_) {
