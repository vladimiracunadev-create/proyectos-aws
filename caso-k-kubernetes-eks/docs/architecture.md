...
portable multi-cloud, Helm para apps complejas, o tu empresa ya tiene skills ## 📐 Diagrama 1: Arquitectura EKS Completa

```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'primaryColor': '#326CE5', 'secondaryColor': '#FF9900', 'tertiaryColor': '#f4f4f4', 'fontsize': '16px' }}}%%
graph TB
    ...
    subgraph EKS_Cluster["☸️ AWS EKS Cluster (us-east-1)"]
        ...
        subgraph NodeGroup["🖥️ Node Group (Managed EC2)"]
            direction TB
    ...
    style LB fill:#FF9900,color:#fff,stroke:#e68a00,stroke-width:2px
    style ControlPlane fill:#326CE5,color:#fff
    style NodeGroup fill:#f5f5f5,stroke:#333
...
```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'primaryColor': '#6C4DE6', 'fontsize': '16px' }}}%%
graph TB
    ...
    subgraph EKS["☸️ EKS Infrastructure"]
        direction TB
    ...
    Agent -->|"forwarding"| API_K8s
    API_K8s --> Rollout

    style Agent fill:#6C4DE6,color:#fff
    style API_K8s fill:#326CE5,color:#fff,stroke:#1a4da1,stroke-width:2px
...
graph TB
    ...
    subgraph HPA_Controller["⚡ HPA Controller"]
        direction TB
        Check["Cada 15s:\ncurrent_replicas × (current/target)"]
        Decision{"¿CPU > 70%?"}
    end

    subgraph DeploymentScale["📋 Deployment Scaling"]
        direction TB
        Pods_2["🟢 replicas: 2 (mín)"]
        Pods_5["🟡 replicas: 5 (scaling)"]
        Pods_10["🔴 replicas: 10 (máx)"]
    end

    CPU --> Check
    Check --> Decision
    Decision -->|"Sí: Scale Up"| Pods_5
    Decision -->|"No: mantener"| Pods_2
